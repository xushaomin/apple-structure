...
public class KDTree<T> implements Serializable {
    ...
    */
	 ...
	 */
	public Set<T> nearestEuclideanReturnSet(double[] key) throws KeySizeException {
		return nearestDistanceReturnSet(key, new EuclideanDistance());
	}
    ...
    */
	...
	@SuppressWarnings("unused")
	private Set<T> nearestDistanceReturnSet(double[] key, DistanceMetric metric) throws KeySizeException {
		NearestNeighborList<KDNode<T>> nnl = getnbrs(key);
		int n = nnl.getSize();
		Set<T> nbrs = new HashSet<T>();
		for (int i = 0; i < n; ++i) {
			KDNode<T> kd = nnl.removeHighest();
			HPoint p = kd.k;
			//kd.
