...
public class Node implements Serializable {
	...
	int nodeId = 0;
	double mbrMinX = Float.MAX_VALUE;
	double mbrMinY = Float.MAX_VALUE;
	double mbrMaxX = -Float.MAX_VALUE;
	double mbrMaxY = -Float.MAX_VALUE;
	...
	int findEntry(double minX, double minY, double maxX, double maxY, int id) {
		for (int i = 0; i < entryCount; i++) {
			if (id == ids[i] && entriesMinX[i] == minX && entriesMinY[i] == minY && entriesMaxX[i] == maxX
					&& entriesMaxY[i] == maxY) {
				return i;
			}
		}
